...
public class TestCommonConnectorMetrics {
  ...
  private static final DynamicMetricsManager METRICS_MANAGER = DynamicMetricsManager.createInstance(new MetricRegistry(), true);
  ...
  public void testConnectorEventProcessingMetrics() {
    ...
    Assert.assertEquals(((Meter) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER1_NAME
        + CommonConnectorMetrics.EventProcMetrics.EVENTS_PROCESSED_RATE)).getCount(), 10);
    Assert.assertEquals(((Meter) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER2_NAME
        + CommonConnectorMetrics.EventProcMetrics.EVENTS_PROCESSED_RATE)).getCount(), 15);
    Assert.assertEquals(((Meter) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_AGGREGATE
  ...
  public void testConnectorPollMetrics() {
    ...
    Assert.assertEquals(((Histogram) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER1_NAME
        + CommonConnectorMetrics.PollMetrics.EVENT_COUNTS_PER_POLL)).getSnapshot()
        .get99thPercentile(), 9.0);
    Assert.assertEquals(((Histogram) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER2_NAME
        + CommonConnectorMetrics.PollMetrics.EVENT_COUNTS_PER_POLL)).getSnapshot()
        .get99thPercentile(), 45.0);
    Assert.assertEquals(((Counter) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_AGGREGATE
  ...
  public void testPConnectorPartitionMetrics() {
    ...
    Assert.assertEquals((long) ((Gauge) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER1_NAME
            + CommonConnectorMetrics.PartitionMetrics.STUCK_PARTITIONS)).getValue(),
        10);
    Assert.assertEquals((long) ((Gauge) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_CONSUMER2_NAME
            + CommonConnectorMetrics.PartitionMetrics.STUCK_PARTITIONS)).getValue(),
        20);
    Assert.assertEquals((long) ((Gauge) METRICS_MANAGER.getMetric(CLASS_NAME + DELIMITED_AGGREGATE
  ...
  public void testDeregisterMetrics() {
    ...
    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, "aggregate", "stuckPartitions")));
    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER1_NAME, "stuckPartitions")));
    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER2_NAME, "stuckPartitions")));

    metrics1.deregisterMetrics();
    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, "aggregate", "stuckPartitions")));
    Assert.assertNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER1_NAME, "stuckPartitions")));

    metrics2.deregisterMetrics();
    Assert.assertNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, "aggregate", "stuckPartitions")));
    Assert.assertNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER2_NAME, "stuckPartitions")));

    CommonConnectorMetrics metrics3 = new CommonConnectorMetrics(CLASS_NAME, CONSUMER3_NAME, LOG);
    metrics3.createPartitionMetrics();

    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, "aggregate", "stuckPartitions")));
    Assert.assertNotNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER3_NAME, "stuckPartitions")));

    metrics3.deregisterMetrics();
    Assert.assertNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, "aggregate", "stuckPartitions")));
    Assert.assertNull(METRICS_MANAGER.getMetric(String.join(".", CLASS_NAME, CONSUMER3_NAME, "stuckPartitions")));
  }
}