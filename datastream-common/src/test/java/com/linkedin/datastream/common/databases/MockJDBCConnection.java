...
public class MockJDBCConnection implements Connection {
  ...
  public PreparedStatement prepareStatement(String sql) throws SQLException {
    Assert.assertTrue(_firstStmt == null || _nextStmt == null, "Statement not null");
    Assert.assertNotNull(sql, "Query is null");
    Assert.assertNotEquals(sql, "", "Query is empty");
    if (_firstCall) {
      _firstCall = false;
      _firstStmt = new MockJDBCPreparedStatement(sql, 0, _keyMap, _dataMap);
      return _firstStmt;
    } else {
      // Ignore the first index in the keyMap and dataMap since the first query
      // is different from subsequent queries and uses a different preparedStatement.
      _nextStmt = new MockJDBCPreparedStatement(sql, 1, _keyMap, _dataMap);
      return _nextStmt;
    }
  }
