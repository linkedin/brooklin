  ...
  public static SchemaBuilder.FieldAssembler<Schema> getFieldAssembler(Boolean isNotNullable, Schema fieldSchema, Schema.Field field, SchemaBuilder.FieldAssembler<Schema> fieldAssembler) {
    ...
    if (typeName.equalsIgnoreCase("ARRAY")) {
      ...
      if (fieldSchema.getName().equalsIgnoreCase("ScribeHeader") || fieldSchema.getName().equalsIgnoreCase("ProductManufacturerPart")) {
        ...
        Schema recordSchema = generateParquetStructuredAvroSchema(fieldSchema, fieldSchema.getName());
...
//        // For list of nested objects
//        if (typeSchema.getElementType().getType().getName().equalsIgnoreCase("RECORD")) {
//          Schema elementTypeSchema = generateFlattenedHeaderParquetStructuredAvroSchema(typeSchema.getElementType(), typeSchema.getElementType().getName());
//          fieldAssembler.name(field.name())
//              .doc(field.doc())
//              .type()
//              .optional()
...
//              }
//            } else {
//              fieldAssembler = getFieldAssembler(isNotNullable, headerField.schema(), headerField, fieldAssembler);
//            }
//
    ...
    public static GenericRecord generateParquetStructuredAvroData(Schema schema, GenericRecord avroRecord) {
        ...
        for (Schema.Field field : fields) {
          ...
          if (avroRecord.get(fieldName) != null) {
            ...
            avroFieldSchemaTypes = getSchemaTypes("manufacturerPart", avroRecord);
          }
              ...
              if (!avroTypeSchema.getType().getName().equalsIgnoreCase("null")) {
                ...
                if (avroTypeName.equalsIgnoreCase("ARRAY")) {
                  if (avroTypeSchema.getElementType().getType().getName().equalsIgnoreCase("RECORD")) {
                    ...
                    // In order to overcome warning: [unchecked] unchecked cast, supressing it as we are sure that it will be array<record>
                  ...
                  // add code to convert fields inside nested object to parquet data
                  //GenericRecord nestedParquetFieldGenericRecord = generateParquetStructuredAvroData(((GenericData.Record) nestedRecord).getSchema().getField(fieldName).schema(), (GenericRecord)nestedRecord.get(fieldName));
